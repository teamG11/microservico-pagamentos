...
class ProdutoController {
  ...
  async criar(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      const criaProdutoUseCase = CriaProdutoUseCaseFactory(
        this.produtoRepository
      );
      const { produto } = await criaProdutoUseCase.executarAsync(
        produtoToCreate
      );
  ...
  async editar(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      const { produto } = await editaProdutoUseCase.executarAsync({
        id,
        ...produtoToUpdate,
      });
  ...
  async remover(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      const removeProdutoUseCase = RemoveProdutoUseCaseFactory(
        this.produtoRepository
      );
  ...
  async buscarPorId(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      const buscaProdutoUseCase = BuscaProdutoUseCaseFactory(
        this.produtoRepository
      );
  ...
  async buscarPorCategoria(
    ...
    try {
      ...
      const buscaProdutoPorCategoria = BuscaProdutosPorCategoriaUseCaseFactory(
        this.produtoRepository
      );
      const produtos = await buscaProdutoPorCategoria.executarAsync({
        categoria,
      });
  ...
  async buscarTodos(request: Request, response: Response, next: NextFunction) {
    try {
      const buscaTodosProdutosUseCase = BuscaTodosProdutosUseCaseFactory(
        this.produtoRepository
      );
